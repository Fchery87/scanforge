...
def upgrade() -> None:
    op.add_column("finding_instances", sa.Column("occurrence_fingerprint", sa.String(length=64), nullable=True))
    ...
    op.create_unique_constraint(
        "uq_finding_instance_scan_occurrence", "finding_instances", ["scan_id", "occurrence_fingerprint"]
    )


def downgrade() -> None:
    op.drop_constraint("uq_finding_instance_scan_occurrence", "finding_instances", type_="unique")
